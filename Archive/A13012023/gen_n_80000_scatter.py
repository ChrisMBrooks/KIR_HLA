...
for index, row in ols_results.iterrows():
    ...
    try:
        ...
        neg_log_p_score_records.append(np.zeros(6))
        failed_immunophenotypes.append([feature_name] + list(beta_hat))
        #print('Failed to transform {} values.'.format(feature_name))

neg_log_p_score_records = np.array(neg_log_p_score_records)

Z = neg_log_p_score_records
...
ticks = ['const', '2DL1', '2DL2s', '2DL2w', '2DL3', '3DL1']
...
plt.savefig('n_80000_scatter_plot.png')

sig_phenos_df = pd.DataFrame(significant_immunophenotypes, columns=['feature_name', 'nlog_p_0', 'nlog_p_1', 'nlog_p_2', 
'nlog_p_3' ,'nlog_p_4' ,'nlog_p_5'])

sig_phenos_df.to_csv('n_80000_signif_immunophenotypes.csv')

failed_phenos_df = pd.DataFrame(failed_immunophenotypes, columns=['feature_name', 'beta_0', 'beta_1', 'beta_2', 
'beta_3' ,'beta_4' ,'beta_5'])

failed_phenos_df.to_csv('n_80000_failed_immunophenotypes.csv')

['const', '2DL1', '2DL2s', '2DL2w', '2DL3', '3DL1']