...
if use_full_dataset:
    filename = 'Data/158b+_trait_ids.csv'
...
gene_indicator_names = ['kir2dl2', 'kir2dl3']

# CD158a - KIR2DL1
# CD158b1 - KIR2DL2 or KIR2DL3

X0 = X[['public_id'] + gene_indicator_names].copy()
Y0 = Y[['public_id'] + relevant_phenos].copy()
Z0 = X0.merge(Y0, on='public_id', how='inner')
Z1 = Z0.dropna()

Z1['kir2dl2or3_count'] = Z1[gene_indicator_names[0]] + Z1[gene_indicator_names[1]]

X1 = Z1['kir2dl2or3_count'].values.reshape(-1,1)
...
ticks = ['const'] + ['kir2dl2/3_count']
ax[0].set_xticklabels(ticks)
ax[1].set_xticklabels(ticks)

fig.supxlabel('kir2dl2/3')
fig.supylabel('-log10(p-value)')

plt.plot([0, 6], [cutoff, cutoff], 'r-')
plt.savefig('Analysis/cd158b+_signif_plot.png')
plt.clf()
plt.cla()
...
#for i in range(0, 1, Y1.shape[1]):
    ...
    plt.savefig('Analysis/kir2dl2or3_{}_ols_scatter_plot.png'.format(relevant_phenos[i]))