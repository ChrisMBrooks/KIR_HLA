...
def get_univariate_anlysis_df(write_to_table:bool):
    ...
    for feature_name in Y.columns[1:-2]:
        ...
        Z1 = Z0[[ind_va_lbl, feature_name]].values
        X1 = Z1[:, 0]
        Y1 = Z1[:, 1]
...
def generate_scatter(ols_results:pd.DataFrame, filename:str):
    ...
    plt.title('Univariate Null Hypothesis Significance Testing - f2DL1')
...
filename = 'Analysis/Univariate/univar_f2DL1_p_value_scatter_{}.png'.format(date_str)
