...
metric = 'f_kir_score' #'kir_count'
partition = 'training'
table = 'model_result_ols' #'model_result_ols_kir_count'
...
for feature_name in phenos_subset:
    ...
    if not np.isnan(p_scores[1]):
        results.append([run_id, feature_name] + beta_hat + [p_scores[1]])
    else: 
        print('univariate failed: ', feature_name)
...
data_sci_mgr.data_mgr.insert_df_to_sql_table(df=results_df, columns=columns, schema='KIR_HLA_STUDY', 
    table=table, use_batches=True, batch_size=5000)
